...
| `annex-n-participant-split.md` | Schema fix — same split applied to `annex_n_activities`. Identical root cause and fix to Annex J. Frontend and Excel parser were already correct. | You're working on Annex N data or notice participant columns always 0. |
...
| `annex-g-submissions-list-not-showing.md` | Bug fix — Annex G not showing in Submissions List, expand tables empty, and editor Actions column broken. Six bugs total: wrong `batch_id` mapping, wrong renderer config key (`batch` → `form_data`), incomplete `form_data` response, invented column field names, AG Grid v33+ HTML string `cellRenderer` breaking icon buttons, dead `useRef` on non-forwardRef component. | You're touching Annex G (editor or viewer), `ANNEX_G_CONFIG`, `getSubmissions()`, or any annex using `submission_id` instead of `batch_id`. Also read before using HTML string `cellRenderer` in any AG Grid editor. |
| `annex-m-services-statistics-fixes.md` | Two UI bugs fixed: (1) spurious "Category" dropdown removed from the Services sub-table (column exists in DB/Excel for flat layout only — UI already groups by section). (2) Statistics categories B, C, D were blank/absent after first save because raw DB rows were set directly without merging against the full structure. `mergeStatisticsWithStructure()` now ensures all 4 categories always render with correct fixed/user rows. | You're touching Annex M statistics rendering, the services table UI, or `AnnexMCreate.jsx`. |
