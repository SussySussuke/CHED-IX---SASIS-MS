...
Two-step import: (1) parse + conflict-detect → return summary to frontend, (2) user resolves conflicts sequentially → persist in single DB transaction. Conflict resolution shows a full side-by-side `CompareModal` diff so the user sees exactly what will be overwritten before approving.
...
**Annex F:** Tag row 1, title block rows 3–6 (`ANNEX "F"`, list of programs, student discipline, AY), spacer row 7, activity table header row 8, data row 9+, key-value fields (committee/procedure/complaint desk) written **below the last activity row at a dynamic offset** — col A (merged A:B) = label, col C = value. Parser must scan by label match, not fixed row. Date column is nullable in DB — missing date is not a hard parse error.
...
- `resources/js/Components/Submissions/ImportConflictModal.jsx` — step-through conflict resolver with full side-by-side comparison
- `resources/js/Components/Submissions/CompareModal.jsx` — side-by-side diff viewer used by both submission list and import conflict flow
...
- `serializeExisting()` in `ExcelImportService` uses explicit `->only([...])` whitelists — no raw column/table names reach the frontend. Intentional injection safety measure.
- `CompareModal` expects `{ annex, academic_year, data }` shaped batches. In the import context, `annex` is derived by stripping the `ANNEX_` prefix and hyphenating variant suffixes (C1→C-1, I1→I-1, etc.). No API fetch is needed — data arrives inline in the parse response.
- `CompareModal` is rendered at `z-60` inside a `relative` wrapper to stack above `ImportConflictModal` at `z-50`.
...
- Existing record data serialized with explicit field whitelists only — never `->toArray()` on the full model. This is the canonical safe pattern for any future serialization in this file.
...
- `AnnexFParser` was written assuming KV fields (committee/procedure/complaint desk) are at fixed rows 5–7 col 2. The export actually writes them below the activity table at a dynamic row (col A label, col C value). Old parser silently read wrong rows and discarded KV data. New parser scans every row by label-text match. Additionally, date is `nullable` in `annex_f_activities` but the old parser hard-errored on missing dates — removed that validation.

## Key Discoveries (continued)
- `serializeExisting()` field whitelists were written against a stale/different schema and never updated when field names were standardized to match parsers and `formConfig.js`. All 11 affected annexes (A, B, C, C-1, E, I, I-1, J, K, L, L-1, N, N-1, O) had completely wrong field names, causing the "Current" side of `CompareModal` during import conflicts to render blank or missing columns. F, G, H, M were correct. The parser output fields are the canonical reference — `serializeExisting()` must match them exactly.
- `ANNEX_C` and `ANNEX_C1` cannot share the same `->only()` whitelist as A/B because C has no `target_group` column.
- `ANNEX_F` `serializeExisting()` used `activity_title`, `objectives`, `budget` etc. — the actual DB columns in `annex_f_activities` are `activity`, `date`, `status` only.
- Parser payloads for D and G were flat at root, but their renderers (`ANNEX_D_CONFIG`, `ANNEX_G_CONFIG` in `sharedRendererConfig.js`) expect a wrapper key (`submission` for D, `form_data` for G). `serializeExisting()` for both was already wrapping correctly — only the parsers were not. Fixed in `AnnexDParser` and `AnnexGParser` by wrapping `compact()` under the correct key. The wrapper key must match `formSections[].key` / `formSection.key` in the renderer config — this is the contract between parser payload and renderer.
