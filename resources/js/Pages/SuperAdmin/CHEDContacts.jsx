...
const CHEDContacts = ({ contacts = [] }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditMode && editingContact) {
      put(`/superadmin/ched-contacts/${editingContact.id}`, {
        onSuccess: () => {
  ...
  const handleDelete = () => {
    destroy(`/superadmin/ched-contacts/${deleteConfirm.id}`, {
      onSuccess: () => {
  ...
  const moveContact = (contactId, direction) => {
    const sorted = [...localContacts].sort((a, b) => a.order - b.order);
    const currentIndex = sorted.findIndex((c) => c.id === contactId);
    ...
    const reordered = sorted.map((contact, idx) => ({ ...contact, order: idx + 1 }));
  ...
  const saveOrderChanges = async () => {
    setSavingOrder(true);
    try {
      await axios.post('/superadmin/ched-contacts/reorder', {
        contacts: localContacts.map((c) => ({ id: c.id, order: c.order })),
      });
  ...
  const columnDefs = [
    {
      ...
      cellRenderer: (params) => {
        ...
        const sorted = [...localContacts].sort((a, b) => a.order - b.order);
        const currentIndex = sorted.findIndex((c) => c.id === row.id);
        const isFirst = currentIndex === 0;
        const isLast = currentIndex === sorted.length - 1;

        return (
          <div className="flex items-center justify-center gap-1">
            <IconButton variant="blue" onClick={() => moveContact(row.id, 'up')} disabled={isFirst} title="Move up">
              <IoArrowUp size={16} />
            </IconButton>
            <IconButton variant="blue" onClick={() => moveContact(row.id, 'down')} disabled={isLast} title="Move down">
              <IoArrowDown size={16} />
            </IconButton>
          </div>
        );
      ...
      cellRenderer: (params) => {
        ...
        return (
          <div className="flex items-center justify-center gap-2">
            <IconButton variant="blue" onClick={() => handleEdit(row)} title="Edit Contact">
              <IoPencilOutline size={18} />
            </IconButton>
            <IconButton variant="red" onClick={() => setDeleteConfirm(row)} title="Delete Contact">
  ...
  return (
    <SuperAdminLayout title="CHED Contacts">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button
                      type="submit"
                      disabled={processing}
                      className="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:w-auto sm:text-sm disabled:opacity-50 transition-colors"
                    >
                      {processing
