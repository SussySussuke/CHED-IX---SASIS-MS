...
const Create = ({ availableYears = [], existingBatches = {}, defaultYear, isEditing = false }) => {
  ...
  const initialData = activities && activities.length > 0
    ? activities.map(a => ({
        ...
        implementation_date: a.implementation_date ? a.implementation_date.split('T')[0] : '',
  ...
  const columns = [
    { data: 'title_of_activity', title: 'Title of Activity', type: 'text', width: 300, placeholder: 'Activity title' },
    { data: 'implementation_date', title: 'Implementation Date', type: 'date', dateFormat: 'YYYY-MM-DD', correctFormat: true, width: 150, placeholder: 'YYYY-MM-DD' },
    { data: 'implementation_venue', title: 'Implementation Venue', type: 'text', width: 250, placeholder: 'Venue' },
    { data: 'number_of_participants', title: 'Number of Participants', type: 'numeric', width: 150, placeholder: '0' },
    { data: 'organizer', title: 'Organizer', type: 'text', width: 200, placeholder: 'Organizer' },
  ...
  const handleSubmit = () => {
    ...
    for (let i = 0; i < tableData.length; i++) {
      const [titleOfActivity, implementationDate, implementationVenue, numberOfParticipants, organizer, remarks] = tableData[i];
      if (!titleOfActivity && !implementationDate && !implementationVenue && !numberOfParticipants && !organizer && !remarks) continue;
      if (!titleOfActivity || !implementationDate || !implementationVenue || numberOfParticipants === null || !organizer) {
        alert(`Row ${i + 1}: Please fill in all required fields (Title, Date, Venue, Number of Participants, Organizer)`);
        return;
      }
      activities.push({
        title_of_activity: titleOfActivity,
        implementation_date: implementationDate,
        implementation_venue: implementationVenue,
    ...
    setProcessing(true);
    router.post('/hei/annex-n', { academic_year: academicYear, activities, request_notes: requestNotes }, { onFinish: () => setProcessing(false) });
  };

  return (
    <HEILayout title="Submit Annex N">
      <style>{HOT_TABLE_DARK_MODE_STYLES}</style>
      <div className="space-y-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">CULTURE AND THE ARTS</h1>
            <span className="text-xl font-semibold text-gray-500 dark:text-gray-400">Annex N</span>
          </div>
        </div>
        {!isEditing && (
          <AcademicYearSelect
        ...
        <InfoBox type="info" message="Add multiple culture and arts activities in the table below. You can submit them all at once as a batch." />
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700 space-y-6">
          {shouldLock && (
            <SubmissionLock
              ...
              editUrl={existingBatch?.batch_id ? `/hei/annex-n/edit/${existingBatch.batch_id}` : null}
              historyUrl="/hei/annex-n/history"
            />
          )}
        ...
        <div>
          ...
          <div className="mb-4">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Culture and Arts Activities</h2>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Add multiple activities below. Each row represents one activity.</p>
          </div>
          ...
          <button type="button" onClick={handleAddRow} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
            <IoAddCircle className="text-lg" />Add Another Activity
          </button>
        </div>
          ...
          <div className="flex justify-end gap-4">
            <button type="button" onClick={() => router.visit('/hei/annex-n/history')} className="px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Cancel</button>
