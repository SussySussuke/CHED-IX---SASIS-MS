...
const Create = ({ availableYears = [], existingBatches = {}, defaultYear, isEditing = false }) => {
  ...
  // Form fields
  const [officialSchoolName, setOfficialSchoolName] = useState(formData?.official_school_name || '');
  const [studentPublicationName, setStudentPublicationName] = useState(formData?.student_publication_name || '');
  const [publicationFee, setPublicationFee] = useState(formData?.publication_fee_per_student || '');

  // Frequency checkboxes
  const [freqMonthly, setFreqMonthly] = useState(formData?.frequency_monthly || false);
  const [freqQuarterly, setFreqQuarterly] = useState(formData?.frequency_quarterly || false);
  const [freqAnnual, setFreqAnnual] = useState(formData?.frequency_annual || false);
  const [freqPerSemester, setFreqPerSemester] = useState(formData?.frequency_per_semester || false);
  const [freqOthers, setFreqOthers] = useState(formData?.frequency_others || false);
  const [freqOthersSpecify, setFreqOthersSpecify] = useState(formData?.frequency_others_specify || '');

  // Publication type checkboxes
  const [pubTypeNewsletter, setPubTypeNewsletter] = useState(formData?.publication_type_newsletter || false);
  const [pubTypeGazette, setPubTypeGazette] = useState(formData?.publication_type_gazette || false);
  const [pubTypeMagazine, setPubTypeMagazine] = useState(formData?.publication_type_magazine || false);
  const [pubTypeOthers, setPubTypeOthers] = useState(formData?.publication_type_others || false);
  const [pubTypeOthersSpecify, setPubTypeOthersSpecify] = useState(formData?.publication_type_others_specify || '');

  const [adviserName, setAdviserName] = useState(formData?.adviser_name || '');
  const [adviserPosition, setAdviserPosition] = useState(formData?.adviser_position_designation || '');
        ...
        <div className="relative space-y-6">
          {shouldLock && (
            <SubmissionLock
              ...
              editUrl={existingBatch?.batch_id ? `/hei/annex-g/edit/${existingBatch.batch_id}` : null}
              historyUrl="/hei/annex-g/history"
            />
          )}
