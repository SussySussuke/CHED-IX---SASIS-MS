...
const History = ({ batches }) => {
  ...
  const { showCancelModal, selectedId, handleCancel, handleCancelConfirm, handleCancelClose } = useCancelSubmission('annex-b');
  ...
  const toggleBatch = async (batchId) => {
    if (expandedBatches[batchId]) {
      ...
      if (!batchPrograms[batchId]) {
        ...
        try {
          const response = await fetch(`/hei/annex-b/${batchId}/programs`);
  ...
  return (
    <HEILayout title="Annex B History">
      <style>{HOT_TABLE_DARK_MODE_STYLES}</style>

      <div className="space-y-6">
        <HistoryHeader
          annexName="B"
          submitUrl="/hei/annex-b/submit"
        />

        {batches.length === 0 ? (
          <EmptyState
            title="No Submissions Yet"
            message="You haven't submitted any Annex B data yet."
            buttonText="Create Your First Submission"
            buttonHref="/hei/annex-b/submit"
          />
        ) : (
            ...
            {batches.map((batch) => {
              ...
              return (
                <BatchCard
                  ...
                  editUrl={`/hei/annex-b/${batch.batch_id}/edit`}
