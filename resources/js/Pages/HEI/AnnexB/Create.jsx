...
const Create = ({ availableYears = [], existingBatches = {}, defaultYear, isEditing = false, editingBatch = null }) => {
  ...
  const [requestNotes, setRequestNotes] = useState(editingBatch?.request_notes || '');
  const [processing, setProcessing] = useState(false);
  const [academicYear, setAcademicYear] = useState(currentAcademicYear);

  // Use editingBatch if in edit mode, otherwise use selected year batch
  const existingBatch = isEditing ? editingBatch : existingBatches[selectedYear];
  ...
  // Update data when selected year changes (only in create mode)
  useEffect(() => {
    if (isEditing) return; // Don't switch data in edit mode
  ...
  }, [selectedYear, existingBatches, isEditing]);
  ...
  const handleSubmit = () => {
    ...
    for (let i = 0; i < tableData.length; i++) {
      ...
      const [title, venue, implementation_date, target_group, participants_online, participants_face_to_face, organizer, remarks] = row;

      // Skip completely empty rows
      if (!title && !venue && !implementation_date && !target_group && !organizer) {
        continue;
      }

      // Validate required fields
      if (!title || !venue || !implementation_date || !target_group || !organizer) {
        alert(`Row ${i + 1}: Please fill in all required fields (Title, Venue, Date, Target Group, Organizer)`);
        return;
      }
  ...
  return (
    <HEILayout title="Submit Annex B">
      ...
      <div className="space-y-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
              LIST OF PROGRAMS/ PROJECTS/ ACTIVITIES GUIDANCE AND COUNSELING SERVICE
            </h1>
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700 space-y-6">
          {shouldLock && (
            <SubmissionLock
              ...
              editUrl={existingBatch?.batch_id ? `/hei/annex-b/${existingBatch.batch_id}/edit` : null}
              historyUrl="/hei/annex-b/history"
            />
          )}
        ...
        <div>
          ...
          <div className="flex justify-end gap-4">
            <button
              type="button"
              onClick={() => router.visit('/hei/annex-b/history')}
