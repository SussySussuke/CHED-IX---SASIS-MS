...
const History = ({ batches }) => {
  ...
  const { showCancelModal, selectedId, handleCancel, handleCancelConfirm, handleCancelClose } = useCancelSubmission('annex-c');
  ...
  const toggleBatch = async (batchId) => {
    if (expandedBatches[batchId]) {
      ...
      if (!batchPrograms[batchId]) {
        ...
        try {
          const response = await fetch(`/hei/annex-c/${batchId}/programs`);
  ...
  return (
    <HEILayout title="Annex C History">
      <style>{HOT_TABLE_DARK_MODE_STYLES}</style>

      <div className="space-y-6">
        <HistoryHeader
          annexName="C"
          submitUrl="/hei/annex-c/submit"
        />

        {batches.length === 0 ? (
          <EmptyState
            title="No Submissions Yet"
            message="You haven't submitted any Annex C data yet."
            buttonText="Create Your First Submission"
            buttonHref="/hei/annex-c/submit"
          />
        ) : (
            ...
            {batches.map((batch) => {
              ...
              return (
                <BatchCard
                  ...
                  editUrl={`/hei/annex-c/${batch.batch_id}/edit`}
