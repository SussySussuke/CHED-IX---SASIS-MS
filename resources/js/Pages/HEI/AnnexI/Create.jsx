...
const Create = ({ availableYears = [], existingBatches = {}, defaultYear, isEditing = false }) => {
  ...
  const scholarships = existingBatch?.scholarships || [];
  const hotTableRef = useRef(null);
  const isDark = useDarkMode();
  const [requestNotes, setRequestNotes] = useState('');
  const [processing, setProcessing] = useState(false);

  const initialData = scholarships && scholarships.length > 0
    ? scholarships.map(s => ({
  ...
  const handleSubmit = () => {
    ...
    for (let i = 0; i < tableData.length; i++) {
      ...
      const [scholarshipName, type, categoryIntendedBeneficiaries, numberOfBeneficiaries, remarks] = row;

      if (!scholarshipName && !type && !categoryIntendedBeneficiaries && !numberOfBeneficiaries && !remarks) {
        continue;
      }

      if (!scholarshipName || !type || !categoryIntendedBeneficiaries || numberOfBeneficiaries === null) {
        alert(`Row ${i + 1}: Please fill in all required fields (Scholarship Name, Type, Category, Number of Beneficiaries)`);
        return;
      }
      ...
      scholarships.push({
        ...
        type,
        category_intended_beneficiaries: categoryIntendedBeneficiaries,
    ...
    if (scholarships.length === 0) {
      alert('Please add at least one scholarship to submit.');
      return;
    }

    setProcessing(true);

    router.post('/hei/annex-i', {
  ...
  return (
    <HEILayout title="Submit Annex I">
      ...
      <div className="space-y-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
              LIST OF SCHOLARSHIPS/STUDENT FINANCIAL ASSISTANCE
            </h1>
        ...
        <InfoBox
          type="info"
          message="Add multiple scholarships in the table below. You can submit them all at once as a batch."
        />
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700 space-y-6">
          {shouldLock && (
            <SubmissionLock
              ...
              editUrl={existingBatch?.batch_id ? `/hei/annex-i/edit/${existingBatch.batch_id}` : null}
              historyUrl="/hei/annex-i/history"
            />
          )}
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Add multiple scholarships below. Each row represents one scholarship.
            </p>
          </div>
          ...
          <div className="flex justify-end gap-4">
            <button
              type="button"
              onClick={() => router.visit('/hei/annex-i/history')}
