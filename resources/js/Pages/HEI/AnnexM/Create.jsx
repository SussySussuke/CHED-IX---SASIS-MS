                    ...
                    <React.Fragment key={year}>
                      <td className="px-2 py-2 text-sm text-center border-r border-gray-200 dark:border-gray-600">
                        <input
                          ...
                          onChange={(e) => {
                            const newData = [...statisticsData];
                            if (!newData[index].year_data) newData[index].year_data = {};
                            if (!newData[index].year_data[year]) newData[index].year_data[year] = {};
                            newData[index].year_data[year].enrollment = parseInt(e.target.value) || 0;
                            setStatisticsData(recalculateStatistics(newData));
                          }}
                      ...
                      <td className="px-2 py-2 text-sm text-center border-r border-gray-200 dark:border-gray-600">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (!newData[index].year_data) newData[index].year_data = {};
                            if (!newData[index].year_data[year]) newData[index].year_data[year] = {};
                            newData[index].year_data[year].graduates = parseInt(e.target.value) || 0;
                            setStatisticsData(recalculateStatistics(newData));
                          }}
                          className={`w-full px-2 py-1 text-sm text-center border border-gray-300 dark:border-gray-600 rounded ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700 font-semibold cursor-not-allowed' : 'bg-white dark:bg-gray-800'} text-gray-900 dark:text-gray-100`}
                          min="0"
        ...
        <div className="relative space-y-6">
          {shouldLock && (
            <SubmissionLock
              ...
              editUrl={existingBatch?.batch_id ? `/hei/annex-m/edit/${existingBatch.batch_id}` : null}
              historyUrl="/hei/annex-m/history"
            />
          )}
