...
const Create = ({ availableYears = [], existingSubmissions = {}, defaultYear, isEditing = false }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    post('/hei/general-information');
  };

  return (
    <HEILayout title={isEditing ? "Edit General Information" : "Submit General Information"}>
      <div className="space-y-6">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
          {isEditing ? 'Edit General Information' : 'Submit General Information'}
        </h1>

        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          {shouldLock && (
            <SubmissionLock
              message={existingSubmission?.id ? "This academic year has published data. You can edit it to submit an update request." : "You have already submitted and published data for this academic year."}
              editUrl={existingSubmission?.id ? `/hei/general-information/${existingSubmission.id}/edit` : null}
              historyUrl="/hei/general-information/history"
            />
          )}
          <InfoBox
            type="info"
