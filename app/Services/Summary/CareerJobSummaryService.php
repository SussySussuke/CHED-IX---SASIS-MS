...
class CareerJobSummaryService
{
    ...
    {
        return AnnexCBatch::whereIn('status', ['published', 'submitted', 'request'])
    ...
    public function getRows(string $year): array
    {
        ...
        $batches = AnnexCBatch::where('academic_year', $year)
        ...
        return $heis->map(function ($hei) use ($batches, $year) {
            ...
            $counts = array_fill_keys(
                array_keys(CategoryKeywordMatcher::CAREER_JOB_KEYWORDS),
            ...
            foreach ($batch->programs as $program) {
                ...
                if ($override && !empty($override->manual_categories)) {
                    ...
                    $matched = $this->matcher->matchCareerJob(strtolower($program->title));
    ...
    public function getEvidence(int $heiId, string $category, string $year): array
    {
        ...
        $overrides = CareerJobCategoryOverride::whereIn('program_id', $programs->pluck('id')->toArray())
            ->get()->keyBy('program_id');
        ...
        foreach ($programs as $program) {
            ...
            $assignedCategories = !empty($manualCategories)
                ? $manualCategories
                : $this->resolveCategories(strtolower($program->title));
    ...
    {
        $matched = $this->matcher->matchCareerJob($text);
    ...
    private function emptyRow(HEI $hei, string $year): array
    {
        ...
        foreach (array_keys(CategoryKeywordMatcher::CAREER_JOB_KEYWORDS) as $cat) {
