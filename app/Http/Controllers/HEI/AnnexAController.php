...
class AnnexAController extends Controller
{
    public function create()
    {
        ...
        // Get all submissions for this HEI with programs
        $existingBatches = AnnexABatch::where('hei_id', $heiId)
        ...
        return inertia('HEI/AnnexA/Create', [
    ...
    public function store(Request $request)
    {
        ...
        $existingBatch = AnnexABatch::where('hei_id', $heiId)
            ->where('academic_year', $academicYear)
            ->whereIn('status', ['submitted', 'published', 'request'])
            ->first();

        $newStatus = 'submitted';
        $message = 'Annex A batch submitted successfully! Waiting for publish date.';

        if ($existingBatch) {
            if ($existingBatch->status === 'submitted') {
                // Before publish: overwrite previous submitted
                AnnexABatch::where('hei_id', $heiId)
                    ->where('academic_year', $academicYear)
        ...
        return redirect()->route('hei.annex-a.history')->with('success', $message);
    }

    public function history()
    {
        $batches = AnnexABatch::where('hei_id', Auth::user()->hei_id)
            ->withCount('programs')
            ->orderBy('created_at', 'desc')
            ->get();

        return inertia('HEI/AnnexA/History', [
            'batches' => $batches
        ]);
    }
    ...
    {
        $batch = AnnexABatch::where('batch_id', $batchId)->first();
    ...
    public function edit($batchId)
    {
        ...
        if (!$batch) {
            return redirect()->route('hei.annex-a.history')->withErrors([
    ...
    public function cancel(Request $request, $batchId)
    {
        ...
        return redirect()->route('hei.annex-a.history')->with('success', 'Request cancelled successfully.');
    }
}
