...
class GeneralInformationController extends Controller
{
    public function create()
    {
        ...
        $existingSubmissions = GeneralInformation::where('hei_id', $heiId)
        ...
        return inertia('HEI/GeneralInformation/Create', [
    ...
    public function store(Request $request)
    {
        ...
        $existingSubmission = GeneralInformation::where('hei_id', Auth::user()->hei_id)
        ...
        if ($existingSubmission) {
            if ($existingSubmission->status === 'submitted') {
                ...
                GeneralInformation::where('hei_id', Auth::user()->hei_id)
                    ->where('academic_year', $academicYear)
        ...
        return redirect()->route('hei.general-information.history')->with('success', $message);
    }

    public function history()
    {
        $submissions = GeneralInformation::where('hei_id', Auth::user()->hei_id)
            ->orderBy('created_at', 'desc')
            ->get();

        return inertia('HEI/GeneralInformation/History', [
    ...
    {
        $submission = GeneralInformation::where('id', $id)
    ...
    {
        $submissionToEdit = GeneralInformation::where('id', $id)
            ->where('hei_id', Auth::user()->hei_id)
            ->firstOrFail();
        ...
        // 1. Editing 'submitted' when there's already a pending 'request'
        // 2. Editing an existing 'request' (the one being edited becomes overwritten, new one is created)
        GeneralInformation::where('hei_id', Auth::user()->hei_id)
        ...
        $newSubmission = GeneralInformation::create([
            'hei_id' => Auth::user()->hei_id,
        ...
        return redirect()->route('hei.general-information.history')->with('success', 'Update request submitted successfully! Waiting for admin approval.');
    }
    ...
    public function cancel(Request $request, $id)
    {
        ...
        return redirect()->route('hei.general-information.history')->with('success', 'Request cancelled successfully.');
    }
}
